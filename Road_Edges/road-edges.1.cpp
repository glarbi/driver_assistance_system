...
{   
    ...
    {   
	    ...
	    //   raw_frame = raw_frame(Rect(0, (frame_height-half_height), frame_width, half_height));
	    raw_frame = raw_frame(rect_raw);
...
vector <int> road_edges::car_position(Mat in , Mat out , int vert_nbr ) //vert_nbr should be grater thatn 0 for devision
{
   ...
   vector <Point> vect_right ;
	...
	{	
		int stage = (half_height * vert_pos /vert_nbr) ; // vertical stage 
 	    ...
 	    {    
			...
			Scalar pix = in.at<Vec3b>(stage, x);
	     	if ( pix==Scalar(0,0,255))  // detect red line
	    	{
	...
	for (int vert_pos = 1 ; vert_pos <  vert_nbr; vert_pos++) // vertical eteration to draw circle and lines
	{
