	...
	{	
        ...
        int left_side ,right_side ;
 	    ...
 	    {        
			...
			Scalar hsv_pix   = hsv.at  <Vec3b>(stage, x);
	     	...
	     	if (white_pix == red   )  // detect red line
	    	{
			    Point centre (x,stage) ;
				vect_right .push_back(centre) ;break;    
	       	}
			if (canny_pix == red  )  // detect red line
	    	{
			    Point centre (x,stage) ;
				vect_right .push_back(centre) ;break;    
	       	}
			   if (hsv_pix == red  )  // detect red line
	    	{
			    Point centre (x,stage) ;
				vect_right .push_back(centre) ;break;    
	       	}
		...
		for (int x = half_width; x > 0; x--)  // LEFT  SIDES
     	{ 
	    	...
	    	{
			    Point centre (x,stage) ;
				vect_left.push_back(centre) ;break;    
	       	}
			if (canny_pix == red  )  // detect red line
	    	{
			    Point centre (x,stage) ;
        ...
        /*string m = to_string(left_side);
		putText(out, m, Point(left_side-13 , stage+3), FONT_HERSHEY_SIMPLEX, 0.4, CV_RGB(255, 255, 255), 1.5, 8);

		Point centre (left_side,stage) ;
